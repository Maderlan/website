...
你可以在 [Docker 官网](https://docs.docker.com/engine/install/) 找到相关的安装说明，并使用 [快速上手](/guide/start/quick-start) 中的基本配置将 GZCTF 运行起来。

## 安装 k3s

k3s 是一个轻量级的 k8s 发行版，可以在单机和多机上快速部署 k8s 集群。官网文档地址：[https://docs.k3s.io/zh](https://docs.k3s.io/zh)

:::warning{title="如果你只有一台机器？"}

如果你只有一台机器，我们强烈建议你使用其他的方式进行部署：

- 对于**小于 200 支队伍、30 道题目的私有内网**比赛，可以直接使用 Docker 部署，无需使用 k8s。
- 对于**公开或规模更大的比赛**，强烈建议使用 k3s 一体化部署，**不要使用 Docker**。
